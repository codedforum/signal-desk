...
// Probing an endpoint means calling it the way it actually accepts. Several
// endpoints require an identifier, so a bare limit=1 comes back 400 (or CMC's
// 4002) and the capability page reports a working endpoint as broken. Each entry
// says what a minimal valid call looks like. The ones needing an id resolve it
// from a sibling list endpoint at probe time, because an id pinned here would
// rot the moment the reference asset changed.
const PROBE = {
  quotes:    () => 'symbol=BTC',
  info:      () => 'symbol=BTC',
  category:  (ids) => (ids.categoryId ? `id=${encodeURIComponent(ids.categoryId)}` : null),
  rwaInfo:   (ids) => (ids.rwaId ? `rwa_id=${encodeURIComponent(ids.rwaId)}` : null),
  rwaQuotes: (ids) => (ids.rwaId ? `rwa_id=${encodeURIComponent(ids.rwaId)}` : null),
  rwaIssuer: (ids) => (ids.issuerId ? `issuer_id=${encodeURIComponent(ids.issuerId)}` : null),
};

// Resolve the identifiers those probes need. A failure here is not an endpoint
// failure: it means the probe cannot run, which the caller reports as 'skipped'
// rather than counting it against the key.
async function probeIds() {
  const pick = async (path, keyName, ...fields) => {
    const r = await call(`${path}?limit=1`, { name: `probeid:${keyName}` });
    if (!r.ok) return null;
    const row = rwaRows(r.data, keyName)[0];
    if (!row) return null;
    for (const f of fields) if (row[f] != null) return row[f];
    return null;
  };
  const [categoryId, rwaId, issuerId] = await Promise.all([
    pick(ENDPOINTS.categories.path, 'categories', 'id'),
    pick(ENDPOINTS.rwaList.path, 'assets', 'rwa_id', 'id'),
    pick(ENDPOINTS.rwaIssuers.path, 'issuers', 'issuer_id', 'id'),
  ]);
  return { categoryId, rwaId, issuerId };
}

// The query string for a minimal valid probe, or null when a required id could
// not be resolved.
function probeQuery(name, ids) {
  const f = PROBE[name];
  return f ? f(ids || {}) : 'limit=1';
}

module.exports = {
  ENDPOINTS, RWA_TYPES, call, usage, rwaRows,
  PROBE, probeIds, probeQuery,
